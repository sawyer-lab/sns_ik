...
namespace sns_ik
{
  ...
  SNSPositionIK::SNSPositionIK(const KDL::Chain &chain, std::shared_ptr<SNSVelocityIK> velocity_ik, double eps) : m_chain(chain),
                                    ...
                                    KDL::Vector *rotAxis)
  {
    ...
    *errR = 0.0;                    // No rotation error
    *rotAxis = KDL::Vector::Zero(); // No rotation direction
    return true;
  }
                               ...
                               const KDL::Twist &bounds)
  {
    ...
    for (ii = 0; ii < m_maxIterations; ++ii)
    {
      ...
      //   delta_twist.rot.z(0);
      delta_twist.rot = KDL::Vector::Zero(); // Completely ignore rotation part
      ...
      Eigen::MatrixXd jac_pos = jacobian.data.topRows(3);

      
      Eigen::MatrixXd jac_trimmed = Eigen::MatrixXd::Zero(6, jac_pos.cols());
      jac_trimmed.topRows(3) = jac_pos;

      sot[0].jacobian = jac_trimmed;
